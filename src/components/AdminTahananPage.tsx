...
import { formatIndonesianDate, normalizeDateToYMD } from '../utils/validation';
...
export const AdminTahananPage: React.FC = () => {
  ...
  const handleOpenModal = (item?: Tahanan) => {
    if (item) {
      ...
      setFormData({
        ...item,
        tanggalLahir: normalizeDateToYMD(item.tanggalLahir),
      });
    } else {
      setIsEditing(false);
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-x-6">
              ...
              <Field label="Tanggal Lahir" value={formatIndonesianDate(viewDetail.tanggalLahir)} />
                ...
                <div>
                  ...
                  {formData.tanggalLahir && (
                    <p className="text-[10px] text-emerald-800 font-semibold mt-1">
                      Format: {formatIndonesianDate(formData.tanggalLahir)}
                    </p>
                  )}
                </div>
                <div>
